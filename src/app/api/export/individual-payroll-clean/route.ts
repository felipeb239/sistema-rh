  ...
  try {
    ...
    if (format === 'pdf') {
      ...
      const employeeRubrics = calculateEmployeeRubrics(
        payroll.employee.employeeRubrics.filter(r => r.isActive),
        baseSalary,
        payroll.month,
        payroll.year
      )
      ...
      const lastDay = getLastDayOfMonth(payroll.month, payroll.year)
      ...
      // Gerar uma função para criar cada holerite
      const generatePayrollHTML = (copyTitle: string) => {
        return `
      ...
      const htmlContent = `
        ...
        <head>
          ...
          <title>Holerite - ${payroll.employee?.name}</title>
          <style>
            * {
              margin: 0;
              padding: 0;
        ...
        <body>
          ${generatePayrollHTML('VIA DA EMPRESA')}
          ${generatePayrollHTML('VIA DO FUNCIONÁRIO')}
        </body>
        </html>
      `
      ...
      return new NextResponse(htmlContent, {
        headers: {
          ...
          'Content-Disposition': `inline; filename="holerite-${payroll.employee?.name}-${payroll.month}-${payroll.year}.html"`
        }
      })
    }
